...
const ModalAddWords = ({ setShow }: Props) => {
	...
	const handleOnClick = (evt: FormEvent) => {
		...
		if (!words?.includes(newWord)) {
			...
			const wordsSorted = [...words!, newWord.toLocaleLowerCase()]?.sort(
	...
	return (
		<section className="absolute top-0 left-0 w-full h-full flex z-10 bg-[#0008]">
			<div className="w-11/12 h-4/6 bg-[#1f2020] m-auto z-20 flex flex-col justify-center items-stretch p-2 rounded-lg">
				...
				<main className="flex flex-col gap-2 flex-auto overflow-hidden">
					...
					<div className="flex flex-wrap gap-2 px-2 h-fit overflow-auto">
						{data?.map((word, index) => {
							return (
								<p key={index} className="bg-slate-800 p-2 rounded-md">
									{word}
								</p>
							);
						})}
